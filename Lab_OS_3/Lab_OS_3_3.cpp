...
void* thread_func1(void *arg)
{
  ...
  while(flag1 != 0)
  {
    ...
    else
    {
      ...
      result = write(filedes[1], bufer, sizeof(bufer));
      if(result == -1)
      {
        perror("Error write()");
      }
...
void* thread_func2(void *arg)
{
  ...
  while(flag1 != 0)
  {
    ...
    result = read(filedes[0], bufer, sizeof(bufer));
    if(result == -1)
    {
      perror("Error read()");
    }
...
int main()
{
  ...
  result = fcntl(filedes[0], F_SETFL, O_NONBLOCK);
  if(result == -1)
  {
    perror("Error fcntl(filedes[0])");
  }
  result = fcntl(filedes[1], F_SETFL, O_NONBLOCK);
  if(result == -1)
  {
    perror("Error fcntl(filedes[1])");
  }
